...
function parseTimeInputToDialValues(timeInput, includeHours = false) {
  if (typeof timeInput !== 'string') return null;
  const value = timeInput.trim();
  if (includeHours) {
    const hhMatch = value.match(/^(\d{1,2}):(\d{2}):(\d{2})\.(\d{2})$/);
    if (hhMatch) {
      return {
        hours: Number(hhMatch[1]),
        minutes: Number(hhMatch[2]),
        seconds: Number(hhMatch[3]),
        centiseconds: Number(hhMatch[4]),
      };
    }
  }

  const mmMatch = value.match(/^(\d{1,2}):(\d{2})\.(\d{2})$/);
  if (!mmMatch) return null;
  return {
    hours: 0,
    minutes: Number(mmMatch[1]),
    seconds: Number(mmMatch[2]),
    centiseconds: Number(mmMatch[3]),
  };
}

function isAtLeastOneMonthOlder(olderTimestamp, newerTimestamp) {
  if (typeof olderTimestamp !== 'number' || typeof newerTimestamp !== 'number') return false;
  const threshold = new Date(olderTimestamp);
  threshold.setMonth(threshold.getMonth() + 1);
  return threshold.getTime() <= newerTimestamp;
}

function formatEntryDelta(currentSeconds, previousSeconds) {
...
function SpinDial({ label, value, maxValue, onChange, resetSignal, accentColor, dialBackgroundColor, compact = false }) {
  ...
  React.useEffect(() => {
    ...
    forceRender();
  }, [CENTER_OFFSET, ITEM_HEIGHT, resetSignal]);
  ...
  React.useEffect(() => {
    ...
    const rawTarget = -(value * ITEM_HEIGHT + CENTER_OFFSET);
    let target = rawTarget;
    while (target - current > PERIOD / 2) target -= PERIOD;
    while (current - target > PERIOD / 2) target += PERIOD;
    ...
    const animateToValue = () => {
      ...
      positionRef.current = normalizePosition(current + diff * eased);
      forceRender();

      if (progress < 1) {
        animationFrameRef.current = requestAnimationFrame(animateToValue);
      }
    };

    animationFrameRef.current = requestAnimationFrame(animateToValue);
  }, [CENTER_OFFSET, ITEM_HEIGHT, PERIOD, normalizePosition, value]);
...
export default function App() {
  ...
  const prefillTimeoutRef = React.useRef(null);
  ...
  useEffect(() => {
    ...
    const loadPersistedState = async () => {
      try {
        const storedTimesByEvent = await AsyncStorage.getItem(STORAGE_KEYS.timesByEvent);
        if (storedTimesByEvent) {
          const parsedTimes = JSON.parse(storedTimesByEvent);
          if (isMounted && parsedTimes && typeof parsedTimes === 'object') {
            setTimesByEvent(parsedTimes);
          }
        }

        const storedCustomEvents = await AsyncStorage.getItem(STORAGE_KEYS.customEvents);
        if (storedCustomEvents) {
          const parsedCustomEvents = JSON.parse(storedCustomEvents);
          if (isMounted && Array.isArray(parsedCustomEvents)) {
            setCustomEvents(parsedCustomEvents);
          }
        }

        const storedUserName = await AsyncStorage.getItem(STORAGE_KEYS.userName);
        if (isMounted && typeof storedUserName === 'string') {
          setUserName(storedUserName);
          setNameDraft(storedUserName);
        }

        const storedIsDarkMode = await AsyncStorage.getItem(STORAGE_KEYS.isDarkMode);
        if (isMounted && (storedIsDarkMode === 'true' || storedIsDarkMode === 'false')) {
          setIsDarkMode(storedIsDarkMode === 'true');
        }
      } catch (error) {
        console.warn('Failed to load persisted app state', error);
      } finally {
        if (isMounted) setHasHydratedStorage(true);
      }
    };
  ...
  useEffect(() => {
    ...
    const saveTimes = async () => {
      try {
        await AsyncStorage.setItem(STORAGE_KEYS.timesByEvent, JSON.stringify(timesByEvent));
      } catch (error) {
        console.warn('Failed to save timesByEvent', error);
      }
    };
    saveTimes();
  }, [hasHydratedStorage, timesByEvent]);

  useEffect(() => {
    if (!hasHydratedStorage) return;
    const saveCustomEvents = async () => {
      try {
        await AsyncStorage.setItem(STORAGE_KEYS.customEvents, JSON.stringify(customEvents));
      } catch (error) {
        console.warn('Failed to save customEvents', error);
      }
    };
    saveCustomEvents();
  }, [customEvents, hasHydratedStorage]);
  ...
  useEffect(() => {
    ...
    saveUserName();
  }, [hasHydratedStorage, userName]);

  useEffect(() => {
    if (!hasHydratedStorage) return;
    const saveTheme = async () => {
      try {
        await AsyncStorage.setItem(STORAGE_KEYS.isDarkMode, String(isDarkMode));
      } catch (error) {
        console.warn('Failed to save isDarkMode', error);
      }
    };
    saveTheme();
  }, [hasHydratedStorage, isDarkMode]);
  ...
  useEffect(() => {
    if (!hasHydratedStorage || hasPromptedForName) return;
    if (!userName.trim()) {
      setIsNamePromptVisible(true);
    }
    setHasPromptedForName(true);
  }, [hasHydratedStorage, hasPromptedForName, userName]);
  ...
  useEffect(() => {
    ...
    const entries = timesByEvent[selectedEvent] || [];
    if (entries.length === 0) return;

    const latest = sortEntriesByDateAndTime(entries)[0] || null;
    const fastest = entries.reduce((best, entry) => (entry.timeSeconds < best.timeSeconds ? entry : best), entries[0]);

    let entryToPrefill = fastest;
    if (
      fastest &&
      latest &&
      fastest.id !== latest.id &&
      isAtLeastOneMonthOlder(fastest.dateTimestamp, latest.dateTimestamp)
    ) {
      entryToPrefill = latest;
    }

    const parsedDialValues = parseTimeInputToDialValues(entryToPrefill?.timeInput || '', eventNeedsHourDial(selectedEvent));
    if (!parsedDialValues) return;
    ...
    prefillTimeoutRef.current = setTimeout(() => {
      setTimeDialValues({
        hours: parsedDialValues.hours || 0,
        minutes: parsedDialValues.minutes || 0,
        seconds: parsedDialValues.seconds || 0,
        centiseconds: parsedDialValues.centiseconds || 0,
      });
    }, 800);
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.eventScrollContent}>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => {
            ...
            const deltaInfo = previousEntry ? formatEntryDelta(item.timeSeconds, previousEntry.timeSeconds) : null;
            const deltaColor = deltaInfo ? (deltaInfo.delta <= 0 ? '#16a34a' : '#dc2626') : null;
            ...
            return (
              <View style={[styles.historyCard, themedCardStyle]}>
                ...
                {deltaInfo && (
                  <Text style={[styles.historyDelta, { color: deltaColor }]}>{deltaInfo.label}</Text>
                )}
          ...
          <View style={[styles.modalCard, themedCardStyle]}>
            ...
            <View style={styles.calendarGrid}>
              {calendarDays.map((dateObj, index) => {
                ...
                const isFutureDate = isoValue > todayIsoDate;
